...
describe('pahCollectionParser', () => {
  ...
  const sourceId = 'development:pah:validated';
  ...
  describe('metadata.annotations', () => {
    ...
    it('should set discovery source ID annotation', () => {
      const collection = createMockCollection();
      const entity = pahCollectionParser({ collection, baseUrl, sourceId });

      expect(
        entity.metadata.annotations!['ansible.io/discovery-source-id'],
      ).toBe('development:pah:validated');
    });

    it('should use provided source ID in annotation', () => {
      const collection = createMockCollection({
        repository_name: 'rh-certified',
      });
      const customSourceId = 'production:pah:rh-certified';
      ...
      expect(
        entity.metadata.annotations!['ansible.io/discovery-source-id'],
      ).toBe('production:pah:rh-certified');
    });
  });
