...
/**
 * Scans a TypeScript source file for:
 *   - createPlugin({ id: '...' })       → family: frontend, currentId: plugin:<id>
 *   - createRouteRef({ id: '...' })     → family: frontend, currentId: routeRef:<id>
 */
export function scanFrontend(
  ...
  for (const call of sf.getDescendantsOfKind(SyntaxKind.CallExpression)) {
    ...
    if (callee !== 'createPlugin' && callee !== 'createRouteRef') continue;
    ...
    rows.push({
      ...
      runtime: 'frontend',
      source,
      fixture: fixtureRef('frontend', currentId),
    });
  }
